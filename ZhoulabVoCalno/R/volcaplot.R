...
#'
#' @title This is first R package to plot volcano plot
#'
#' @author ZhouLab JunJun
#'
...
#' @import ggrepel
#'
#' @param volcaplot The plot function name
#'
...
#' @param col The point color you want to show,this is color list like c('red','blue','gray') stands for up down and no significant genes
...
#' @param chose The list contains palettes
...
volcaplot <- function(res, log2FoldChange, pvalue, size, xcut, ycut,
                      ######################################################
                      # 颜色素材
...
####################################################################
  ...
  if(missing(ycut)) ycut <- ceiling(max(-log10(res$pvalue)) + 2)
...
#####################################################################
  ...
  ggplot(data=res,aes(x=log2FoldChange,y=-log10(pvalue))) +
    ...
    scale_x_continuous(limits = c(-xcut,xcut)) +
